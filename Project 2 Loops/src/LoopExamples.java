 ...
 *it is the formula used for calculating the change in velocity 
 *
 *The fourth method updateHeight is a helper method for simulation
 *it is the formula used for calculating the change in Height
 *
 ...
 *THIS CLASS CAN BE RUN. IT IS HARDCODED TO DO THE SIMULATION WITH AN INITIAL VELOCITY OF 100.
 *SimulationTest can be used if you would like to do it with different velocity values.
 */
...
public class LoopExamples
{
		...
		for (; minLength < maxLength + 1; minLength++)
		{
			for (; minWidth < maxWidth + 1; minWidth++)
			{
				System.out.printf("%-10s %-10s %-10s %n", minLength, minWidth,
						(minLength * minWidth)); // prints in pretty columns
				System.out.println("------------------------------");
			}
	...
	public static void simulation(double initialVelocity)
	{
		...
		System.out.printf("%-8s %-18s %-18s %n", "Time", "Formula Height", "Simulation height");
		System.out.printf("%-8.2f %-18.2f %-18s %n", time, formulaHeight(initialVelocity, time), "0.00");
		time++;
		do
		{
			for (int i = 0; i < 101; i++)
			{
				formulaHeight = formulaHeight(initialVelocity, time);
			...
			time++;
			if (currentHeight > 0)
				System.out.printf("%-8.2f %-18.2f %-18.2f %n", time, formulaHeight,
						currentHeight);
